...
#endif
    (td_void) uapi_board_init();        // 单板初始化
    (td_void) uapi_aes_init();          // AES安全库初始化
    uapi_phy_init();                    // PLC物理层初始化
    uapi_mac_cp_init();                 
...
#endif
#endif
    (td_void) uapi_mac_init();          // PLC MAC初始化
    app_pm_init();
    (td_void) uapi_diag_init();         // 诊断模块初始化 
    (td_void) uapi_mid_init();          // 中间层初始化，主要初始化全网读写NV等模块
...
#endif
    uapi_upg_init();                    // 升级模块初始化
    (td_void) app_srv_init();           // 初始化应用程序服务
#if defined(PRODUCT_CFG_PRODUCT_TYPE_STA)
...
td_u32 app_srv_init(td_void)
{
    ...
    ret = app_ctx_init();           // 初始化应用程序上下文
    if (ret != EXT_ERR_SUCCESS) {
        app_printf("[-ERR-] app_ctx_init() failed.\n");
        return ret;
    }

    ctx = app_get_srv_context();    // 获取应用程序上下文
#if 1
    ret = app_uart_init();
    if (ret != EXT_ERR_SUCCESS) {
...
#endif
    ret = app_res_init(ctx);        // 初始化应用程序资源
    if (ret != EXT_ERR_SUCCESS) {
        app_printf("[-ERR-] app_res_init() failed.\n");
        return ret;
    }

    (td_void) app_plc_chl_init();   // 初始化PLC通道
    app_register_mac_event();       // 注册MAC事件
#if defined(PRODUCT_CFG_PRODUCT_TYPE_CCO)
    ret = app_ni_init();
    if (ret != EXT_ERR_SUCCESS) {
...
td_void app_on_msg_plc_input(EXT_CONST ext_sys_queue_msg *msg)
{
    ...
    app_print_buf("Payload: ", info->payload, info->payload_length);
